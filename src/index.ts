...
const testCommand: ICommand = {
  ...
  action: async (
    file: string,
    options: { prompt?: string; output?: string }
  ): Promise<void> => {
...
const sqlTranslatorCommand: ICommand = {
  ...
  action: async (
    query: string,
    options: { schemaPath?: string; output?: string }
  ): Promise<void> => {
    const config: IConfig = getConfig()
...
const configCommand: ICommand = {
  ...
  args: '<key> <value>',
  options: [],
  action: async (key: keyof IConfig, value: string): Promise<void> =>
...
const getConfigCommand: ICommand = {
  ...
  action: async (): Promise<void> => {
    ...
    console.table(
      Object.keys(config).map((key: string) => ({ key, value: config[key] }))
    )
  }
}
      ...
      try {
        ...
        if (axios.isAxiosError(error)) {
          if ((error as AxiosError).response?.status === 401) {
            ...
            console.log(
              `${chalk.red('✘')} ${chalk.yellow('OpenAI API is down')}`
            )
            return
          }
        }
